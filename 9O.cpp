...
void bfs(std::vector<VertexInfo>& vertices, std::vector<Edge>& edges) {
...
void dfs(int vertex, std::vector<bool>& used, std::vector<Edge>& edges, std::vector<VertexInfo>& vertices) {
  used[vertex] = true;
  for (int num : vertices[vertex].edges_nums) {
    int to = edges[num].to;
...
double dfsDinica(int vertex, double current_flow, std::vector<VertexInfo>& vertices,
              std::vector<Edge>& edges) {
  int last = static_cast<int>(vertices.size()) - 1;
  if (vertex == last) { return current_flow; }
  while (vertices[vertex].ptr < static_cast<int>(vertices[vertex].edges_nums.size())) {
    int e_num = vertices[vertex].edges_nums[vertices[vertex].ptr];
    int to = edges[e_num].to;
    if (vertices[vertex].group + 1 != vertices[to].group || std::abs(edges[e_num].get_left_cap()) < 0.00001 ) {
      vertices[vertex].ptr = vertices[vertex].ptr + 1;
      continue;
    }
    double x = dfsDinica(to, std::min(current_flow, edges[e_num].get_left_cap()), vertices, edges);
...
class network {
 public:
  network(double g, std::vector<std::pair<int, int>>& other_edges, int count_of_vertices,
    ...
    for (int i = 1; i <= count_of_vertices; ++i) {
      ...
      e += 2;
      edges.emplace_back(i, finish, edges_capacity + 2 * g - static_cast<double>(degrees[i]), 0.0);
  ...
  std::vector<int> fin_min_cut() {
    double max_start_flow = 100000.0;
    double max_flow = 0;
    while (true) {
      bfs(vertices, edges);
      if (vertices[finish].group == -1) { break; }
      for (int i = 0; i <= finish; ++i) {
        vertices[i].ptr = 0;
      }
      while (true) {
        double x = dfsDinica(0, max_start_flow, vertices, edges);
        max_flow += x;
        if (std::abs(x) < 0.00001) {
          break;
        }
...
int main() {
  ...
  auto minimal_distance = 1.0 / static_cast<double>(count_of_vertices * count_of_vertices - count_of_vertices);
  ...
  while (right - left >= minimal_distance) {
    ...
    auto vertices_in_min_cut = net.fin_min_cut();
