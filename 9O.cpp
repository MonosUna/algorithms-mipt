...
class Network {
 ...
 private:
  struct Edge {
    ...
    explicit Edge(int from, int to, double cap, double flow)
...
void Network::dfs(int vertex, std::vector<bool>& used) {
  used[vertex] = true;
  for (int num : vertices[vertex].edges_nums) {
    int to = edges[num].to;
...
double Network::dfsAddingFlow(int vertex, double current_flow) {
...
Network::Network(double g, const std::vector<std::pair<int, int>>& other_edges,
                 int count_of_vertices, const std::vector<int>& degrees) {
...
std::vector<int> Network::FindMinCut() {
  ...
  while (true) {
    ...
    while (true) {
      double x = dfsAddingFlow(0, max_start_flow);
      max_flow += x;
      if (std::abs(x) < 0.00001) {
        break;
      }
