...
struct Edge {
  ...
  Edge(int from, int to, long long cap, long long cost) : from(from), to(to), cap(cap), flow(0), cost(cost) {}
...
class network {
 public:
  network(std::vector<std::pair<int, int>>& other_edges, int count_of_vertices,
  ...
  std::pair<long long, long long> Dijkstra(std::vector<long long>& potentials) {
